...
export default function DealsClient({ initialDeals }: { initialDeals: any[] }) {
  // Group deals by stage
  const dealsByStage = initialDeals.reduce((acc: any, deal: any) => {
    if (!acc[deal.stage]) acc[deal.stage] = []
    acc[deal.stage].push(deal)
    return acc
  }, {})

  const totalValue = initialDeals.reduce((sum, deal) => sum + (deal.value || 0), 0)
  return (
          ...
          return (
            <div key={stage.id} className="min-w-[300px] w-[300px] flex flex-col">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-medium text-text-secondary">{stage.amount}</span>
              </div>
              ...
              <div className="flex-1 flex flex-col gap-3 bg-bg-primary/50 p-2 rounded-xl border border-border/50 overflow-y-auto">
                {deals.map((deal: any) => (
                  <div 
                    key={deal.id}
                    className="bg-bg-primary p-4 rounded-lg border border-border shadow-sm hover:shadow-md transition-shadow cursor-pointer group"
                    draggable
                  >
                    ...
                    <p className="text-xs text-text-secondary mb-4">{deal.contact?.company || "No Company"}</p>
                ...
                <button className="w-full py-3 mt-1 rounded-lg border border-dashed border-border text-sm font-medium text-text-secondary hover:text-primary hover:border-primary/50 hover:bg-primary/5 transition-colors flex items-center justify-center">
