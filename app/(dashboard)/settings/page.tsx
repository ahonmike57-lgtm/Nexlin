...
import { Palette, Globe, CreditCard, Link as LinkIcon, Building, ShieldCheck, Mail, Save, Settings2, Users, Phone, Cpu, Puzzle } from "lucide-react"
          ...
          <nav className="flex lg:flex-col gap-1 overflow-x-auto pb-4 lg:pb-0">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => {
                  if (["domains", "team", "billing", "branding", "phone-numbers", "mcp"].includes(tab.id)) {
                    window.location.href = `/settings/${tab.id}`;
                    return;
                  }
                  setActiveTab(tab.id)
                }}
